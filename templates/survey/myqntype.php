...
switch ( $question->params->get('choice_type') ) {
    ...
    case 'select':
    	...
    	<select class="form-select" name="ngform[answers][<?php echo $question->id?>][response][]" <?php echo $question->params->get('required') ? 'required="required"' : '';?>>
    		<option value=""><?php echo esc_html__( '- Select an option -', 'ngsurvey-extension-myqntype' );?></option>
    		<?php 
    		foreach ( $question->answers as $answer ) {
    		    ?>
    		    <option 
    ...
    <div class="form-inline">
        <div class="form-group">
        	<input type="text" 
        		...
        		placeholder="<?php echo esc_attr__( $question->params->get('custom_answer_placeholder', 'Other'), 'ngsurvey-extension-myqntype' );?>"
